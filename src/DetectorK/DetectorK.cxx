...
public:
  ...
  //  void Print() {printf("  r=%3.1lf X0=%1.6lf sigPhi=%1.4lf
...
void DetectorK::KillLayerTemporarily(const char* name) {
 ...
 //introduce the option to temporarly kill a layer  
  ...
  CylLayerK* tmp = (CylLayerK*)fLayers.FindObject(name);
  if (!tmp) {
    printf("Layer %s not found - cannot mark as temporarily dead\n", name);
    return;
  }
  if (!(tmp->isTempDead)) {
    if (!tmp->hasStoredDefaults) {
       // Store original values only once
    tmp->originalPhiRes = tmp->phiRes;
    tmp->originalZRes   = tmp->zRes;
    tmp->originalRadLen = tmp->radL;
    tmp->hasStoredDefaults = true;
    printf("Layer %s, original phiRes %f, zRes %f, radLen %f\n",name, tmp->phiRes,tmp->zRes,tmp->radL );
    }
    ...
    tmp->zRes = 999999;
    tmp->radL = 0;
    printf("Layer %s NEUTRALISED, original phiRes %f, zRes %f, radLen %f\n",name, tmp->phiRes,tmp->zRes,tmp->radL );
    ...
    if (DetectorK::verboseG) {
      printf("Layer %s marked as temporarily dead (φ-gap), fNumberOfActiveITSLayers %d\n", name, fNumberOfActiveITSLayers);
    }
  }
}
...
void DetectorK::ResetTemporaryLayerFlags() {
  ...
  for (int i = 0; i < fLayers.GetEntries(); ++i) {
    ...
    if (layer->isTempDead) {
      ...
      if (layer->hasStoredDefaults) {
       ...
       layer->radL = layer->originalRadLen;
      }
      else {
        printf("⚠️ Warning: layer %s marked as temp-dead but has no stored defaults. Skipping restore.\n",layer->GetName());
      }
  ...
  if (DetectorK::verboseG) {
    printf("ResetTemporaryLayerFlags: Cleared %d temporarily dead layers\n", tempDeadCount);
  }
}
...
void DetectorK::ApplyShiftedPetalGapsToLayers(
  ...
  for (size_t i = 0; i < layerNames.size(); ++i) {
    ...
    layer->ClearPhiGaps(); // Ensure no gaps accumulate
    ...
    printf("Adding %d gaps to layer %s starting at φ = %.3f\n", nPetals,
           name.Data(), TMath::Pi() / nPetals);
  }
}
...
void DetectorK::PrintLayout(Bool_t full) {
  ...
  for (Int_t i = 0; i < fLayers.GetEntries(); i++) {
    ...
    printf("%d. %s \t %03.2f   \t%1.4f\t  ", i, tmp->GetName(), tmp->radius,
           tmp->radL);
    if (tmp->phiRes == RIDICULOUS)
      printf("  -  ");
    else
      printf("%4.1f   ", tmp->phiRes * 10000);
    if (tmp->zRes == RIDICULOUS)
      printf("  -");
    else
      printf("%4.1f", tmp->zRes * 10000);
...
void DetectorK::AddTPC(Float_t phiResMean, Float_t zResMean, Int_t skip) {
  ...
  for (int i = 0; i < kNPassiveBound; i++) {
    AddLayer(Form("tpc_boundary%d", i), rBoundary[i],
             radLBoubdary[i]); // dummy errors
  }
...
Double_t DetectorK::ThetaMCS(Double_t mass, Double_t radLength,
  ...
  // momentum );
  if (radLength > 0)
    theta = 0.0136 * TMath::Sqrt(radLength) / (beta * momentum) *
            (1 + 0.038 * TMath::Log(radLength));
  return (theta);
}
...
Double_t DetectorK::HitDensity(Double_t radius) {
  ...
  if (radius < fMaxRadiusSlowDet) { // Note that IntegratedHitDensity will
                                    // always be minB one event, or more, even
                                    // if integration time => zero.
...
double DetectorK::IntegratedHitDensity(Double_t multiplicity, Double_t radius) {
  ...
  Double_t den = zdcHz * fIntegrationTime / 1000. * multiplicity *
                 Dist(0., radius) / (2. * TMath::Pi() * radius);

  // printf("R M D %f %f %f\n",radius,multiplicity,den);

  // Note that we do not allow the rate*time calculation to fall below one minB
  // event at the vertex.
...
double DetectorK::UpcHitDensity(Double_t radius) {
  ...
  //  Schweda summary tables at RHIC * 'scale' for LHC
  mUPCelectrons = fLhcUPCscale * 5456 / (radius * radius) /
...
double DetectorK::Dist(double z, double r) {
  ...
  // by Yan Lu 12/20/2006, all radii and Z location in centimeters.
  ...
  for (int i = 0; i < nsteps; i++) {
    if (i == nsteps - 1)
      index = 1;
    z0 = -4 * SigmaD + i * dz0;
    dist += index * (dz0 / 3.) * (1 / sqrt(2. * TMath::Pi()) / SigmaD) *
            exp(-z0 * z0 / 2. / SigmaD / SigmaD) *
...
DetectorK::D0IntegratedEfficiency(Double_t pt,
  ...
  for (Int_t k = 0; k < 360; k++) {
    ...
    if (pionindex >= kNptBins)
      pionindex = 399;
    if (pionindex >= 0)
      effp = corrEfficiency[0][pionindex];
    if (pionindex < 0)
      effp = (corrEfficiency[0][1] - corrEfficiency[0][0]) * pionindex +
             corrEfficiency[0][0]; // Extrapolate if reqd
    if (effp < 0)
      effp = 0;

    if (kaonindex >= kNptBins)
      kaonindex = 399;
    if (kaonindex >= 0)
      effk = corrEfficiency[1][kaonindex];
    if (kaonindex < 0)
      effk = (corrEfficiency[1][1] - corrEfficiency[1][0]) * kaonindex +
             corrEfficiency[1][0]; // Extrapolate if reqd
    if (effk < 0)
      effk = 0;

    // Note that we assume that the Kaon Decay efficiency has already been
    // inlcuded in the kaon efficiency used here.
...
void DetectorK::SolveViaBilloir(Int_t flagD0, Int_t print, Bool_t allPt,
  ...
  // static TH1F* hPhiDist = new TH1F("hPhiDist", "Randomized #phi distribution;
  // #phi [rad];Entries", 60, 0, 2*TMath::Pi());
  // static TH1F* hPhiAccepted = new TH1F("hPhiAccepted", "Accepted #phi after
  // IsInGap;#phi [rad];Tracks", 60, 0, 2*TMath::Pi()); static TH1F*
  // hPhiRejected = new TH1F("hPhiRejected", "Rejected #phi (in gap);#phi
  // [rad];Tracks", 60, 0, 2*TMath::Pi()); static TH1F* hPhiRejected_ddd0 = new
  // TH1F("hPhiRejected_ddd0", "Rejected #phi in ddd0", 60, 0, 2*TMath::Pi());
  // static TH1F* hPhiRejected_ddd1 = new TH1F("hPhiRejected_ddd1", "Rejected
  ...
  if (!allPt) { // not the whole pt range -> allows a faster minimization at a
                // defined 'meanpt'
    nPt = 3;
  }
  ...
  mass[0] = PionMass;
  mass[1] = KaonMass; // Loop twice for the D0;  first pi then k

  mass[2] = fParticleMass; // third loop

  Int_t mStart = 0;
  if (!flagD0)
    mStart = 2; // pion and kaon is skipped -> fast mode
  ...
  for (Int_t massloop = mStart; massloop < 3; massloop++) {
    ...
    Int_t counterLayersWithTrackInGap = 0;
      ...
      // ~20 GeV
      //   if (bigRad<61) bigRad=61; // -> min pt around 100 MeV for Bz=0.5T
      //   (don't overdo it ... ;-) )
      fTransMomenta[i] = ptmin * TMath::Exp(dlpt * i);
      // fTransMomenta[i] = ( 0.3*bigRad*TMath::Abs(fBField)*1e-2 ) - 0.08 -
      // (1./fptScale-0.1) + TMath::Power(10,2.3*i/nPt) / fptScale ;
      ...
      // Assume track started at (0,0,0) and shoots out on the X axis, and B
      // field is on the Z axis These are the EndPoint values for y, z, a, b,
      // and d
      ...
      /*     if (applyRotatedGaps) {
              ...
              printf("applyRotatedGaps YES\n");
     // Define target layers and corresponding gap widths
     std::vector<TString> layerNames = { "ddd0", "ddd1", "ddd2" };  // Customize
 as needed std::vector<double> gapWidths   = { TMath::Pi()/8, TMath::Pi()/6,
 TMath::Pi()/4 }; int nPetals = 3;

     double phiShift = gRandom->Uniform(0., 2 * TMath::Pi());

     ApplyShiftedPetalGapsToLayers(layerNames, gapWidths, nPetals, phiShift);

     //if (print > 0)
         printf("Track %d: applied φ-gap shift of %.3f rad\n", i, phiShift);
 }
 */
      ...
      //	double phi_loc = gRandom->Uniform(0., 2 * TMath::Pi());
      // phi_loc = gRandom->Uniform(0., 2 * TMath::Pi());
      // snp = gRandom->Uniform(-0.99, 0.99);
      // 2. Compute snp = sin(phi_loc)
      // snp = TMath::Sin(phi_loc);
      //} while (TMath::Abs(snp) > 0.98 || TMath::Abs(snp - 0.697866) < 1e-3);
      // 3. Prevent instability: clip if too close to ±1
      //	if (TMath::Abs(snp) > 0.9990) {
      //    	snp = 0.9990 * (snp > 0 ? 1.0 : -1.0);
      //	}

      trPars[kY] = 0; // start from Y = 0
      trPars[kZ] = 0; //            Z = 0
      trPars[kSnp] = 0;
      // printf ("Sin(phi_loc) %f, phi_loc %f\n", trPars[kSnp], phi_loc);
      // trPars[kSnp] = snp;                       //            track along X
      // axis at the vertex
      trPars[kTgl] = tgl;         //            dip
      ...
      //
      if (!PropagateToR(&probTr, last->radius + kTrackingMargin, bGauss, 1))
        continue;
        ...
        if (!isVertex) {
          ...
          if (layer->IsInGap(phi, phiShift)) {
            ...
            if (DetectorK::verboseG) {
              ...
              printf("Track skipped layer %s due to shifted φ-gap\n",
                     layer->GetName());
            }
            // continue;
...
*/
          ...
          // if ( TMath::Abs(TMath::Abs(phi)-TMath::Pi()/2)<1e-3) phi +=
          // epsilon; if ( TMath::Abs(TMath::Abs(phi)-TMath::Pi()/2)<1e-3) phi =
          // 0;//TMath::Sign(TMath::Pi()/2 - 1e-3,phi);
        ...
        double measErr2[3] = {layer->phiRes * layer->phiRes, 0,
        ...
        //  note: if apart from MS we want also e.loss correction, the
        //  density*length should be provided as 2nd param
        if (!probTr.CorrectForMeanMaterial(layer->radL, 0, mass[massloop],
                                           kTRUE)) {
      ...
      // Pattern recognition is done .... save values like vertex resolution
      // etc.

      // Convert the Convariance matrix parameters into physical quantities
      // The results are propogated to the previous point but *do not* include
      // the measurement at that point.
      //      deltaPoverP          =
      //      TMath::Sqrt(probTr.GetSigma1Pt2())/probTr.Get1P();  // Absolute
      //      magnitude so ignore charge
      deltaPoverP = TMath::Sqrt(probTr.GetSigma1Pt2()) /
      ...
      for (Int_t j = lastActiveLayer + 1; j--;) { // Layer loop
        ...
        if ((!isDead && !isTempDead && radLength > 0)) {
          ...
          if (print == 1 && fTransMomenta[i] >= meanPt && massloop == 2 &&
            ...
            printf("%s:\t%5.1f %9.4f %10.0f %11.0f %7.0f %8.0f %8.2f ",
                   ...
                   phiRes * 1.e6, zRes * 1.e6, HitDensity(radius * 100));
            if (!name.Contains("tpc"))
              printf("%10.3f\n", layerEfficiency);
            else
              printf("        -  \n");
       ...
       if (IsITSLayer(layer->GetName())) {
        ...
        // vertex print
        if (print == 1 && fTransMomenta[i] >= meanPt && massloop == 2 &&
        printOnce == 1 && radius==0) { printf("%s:\t -----    ----- %10.0f
        %11.0f \n", layer->GetName(),fDetPointRes[j][i]*1.e6,
        fDetPointZRes[j][i]*1.e6);
        }
        */
      }
      if (print == 1 && fTransMomenta[i] >= meanPt && massloop == 2 &&
      ...
      if (fNumberOfActiveLayers < 1500) {
        // if (fNumberOfActiveLayers < 3 ) {
        //      printf("Backward PtBin%d pt=%f\n",i,pt);

        // BACKWORD TRACKING +++++++++++++++++
        // number of layers is quite low ... efficiency calculation was probably
        // nonsense Tracking outward (backword) to get reliable efficiencies
        // from "smoothed estimates"

        // For below, see paper, NIM A262 (1987) p.444, eqs.12.
        // Equivalently, one can simply combine the forward and backward
        // estimates. Assuming pf,Cf and pb,Cb as extrapolated position
        // estimates and errors from fwd and bwd passes one can use a weighted
        // estimate Cw = (Cf^-1 + Cb^-1)^-1,  pw = Cw (pf Cf^-1 + pb Cb^-1).
        // Surely, for the most extreme point, where one error matrices is
        ...
        if (doLikeAliRoot) {
          ...
          trCov[kPtI2] = kLargeErr2PtI * trPars[kPtI] * trPars[kPtI];
          probTr.CheckCovariance();
          //	  cout<<pt<<": "<<kLargeErr2Coord<<" "<<kLargeErr2Dir<<"
          //"<<kLargeErr2PtI*trPars[kPtI]*trPars[kPtI]<<endl;
        }
        ...
        // values below NOT REALIABLE -> they do not point to the vertex but
        // outwards !!!!!!!
        // ++++++++++++++
        ...
        // Convert the Convariance matrix parameters into physical quantities
        // The results are propogated to the previous point but *do not* include
        the measurement at that point. deltaPoverP    =  TMath::Sqrt(
        mIstar(4,4) ) * momentum / 0.3  ;  // Absolute magnitude so ignore
        charge fMomentumRes[i]      =  100.* TMath::Abs( deltaPoverP ) ;  //
        results in percent fResolutionRPhi[i]      =  TMath::Sqrt( mIstar(0,0) )
        * 1.e6            ;  // result in microns fResolutionZ[i]     =
        TMath::Sqrt( mIstar(1,1) ) * 1.e6            ;  // result in microns
        //      equivalent[i]  = TMath::Sqrt(fResolutionRPhi[i]*fResolutionZ[i])
        ;  // Equivalent circular radius
        */
        ...
        if (print == 1 && fTransMomenta[i] >= meanPt && massloop == 2 &&
            printOnce == 1)
          printf("\n Combined propagation estimates for pT = %f\n",
                 fTransMomenta[i]);
        ...
        for (Int_t j = lastActiveLayer + 1; j--;) { // Layer loop
          ...
          if ((!isDead && !isTempDead && radLength > 0)) {
            ...
            if (massloop == 2 &&
                j == kDetLayer) { // copy layer specific performances
      ...
      // if (!hEffVsPhi)
      // hEffVsPhi = new TH2F("hEffVsPhi", "Efficiency vs #phi and p_{T};#phi
      // [rad];p_{T} [GeV/c];Efficiency",
      //                      60, 0, 2 * TMath::Pi(), kNptBins, ptmin, ptmax);

      // Fill with phi, pt, and calculated efficiency (massloop = 2 is the main
      // one)
      // if (massloop == 2)
      //   hEffVsPhi->Fill(phi_loc, fTransMomenta[i], fEfficiency[massloop][i]);
    ...
    if (fUsePhiRandomisation && hPhiRejected && hPhiAccepted) {
      ...
      cDebugPhi->cd(2);
      hPhiRejected->SetLineColor(kBlack);
      hPhiRejected->Draw();

      cDebugPhi->cd(3);
      hPhiRejected_layer0->SetLineColor(kRed);
      hPhiRejected_layer0->Draw();
      hPhiRejected_layer1->SetLineColor(kBlue);
      hPhiRejected_layer1->Draw("same");
      hPhiRejected_layer2->SetLineColor(kGreen);
      hPhiRejected_layer2->Draw("same");
      ...
      hPhiRejected_layer1->Write();
      hPhiRejected_layer2->Write();
      outf->Close();

      delete cDebugPhi;
      gROOT->SetSelectedPad(nullptr);

      /*TCanvas *cDebugPhi_layer =  new TCanvas("cDebugPhi_layer", "#phi Acceptance Debug", 800, 600);
      cDebugPhi_layer->cd();
      hPhiRejected_layer0->SetLineColor(kRed);
      hPhiRejected_layer0->Draw();
      hPhiRejected_layer1->SetLineColor(kBlue);
      ...
      hPhiRejected_layer2->SetLineColor(kGreen);
      hPhiRejected_layer2->Draw("same");
      cDebugPhi->SaveAs("phi_acceptance_debug_perLayer.png");
      TFile *outf2 = new TFile("phi_debug_gaps_perLayer.root", "RECREATE");
      hPhiRejected_layer0->Write();
      hPhiRejected_layer1->Write();
      hPhiRejected_layer2->Write();
      ...
      delete cDebugPhi_layer;
      gROOT->SetSelectedPad(nullptr);
*/
    }

    /*TCanvas* cLayers = new TCanvas("cLayers", "#phi rejection per layer", 800,
    600); hPhiRejected_ddd0->SetLineColor(kRed);
    hPhiRejected_ddd1->SetLineColor(kBlue);
    hPhiRejected_ddd2->SetLineColor(kGreen);

    hPhiRejected_ddd0->Draw();
    hPhiRejected_ddd1->Draw("SAME");
    hPhiRejected_ddd2->Draw("SAME");

    TLegend* leg = new TLegend(0.7,0.7,0.9,0.9);
    leg->AddEntry(hPhiRejected_ddd0, "ddd0", "l");
    leg->AddEntry(hPhiRejected_ddd1, "ddd1", "l");
    leg->AddEntry(hPhiRejected_ddd2, "ddd2", "l");
    leg->Draw();

    cLayers->SaveAs("phi_rejected_per_layer.png");
...
Bool_t DetectorK::SolveTrack(TrackSol &ts) {
  ...
  //
  // Assume track started at (0,0,0) and shoots out on the X axis, and B field
  ...
  if (!probTr.Rotate(phiL)) {
    printf("Failed to rotate to the frame (phi:%+.3f)of Extertnal layer at
  %.2f\n", phiL,last->radius); probTr.Print(); exit(1);
  }
  */
  if (!probTr.Rotate(probTr.Phi()))
    return kFALSE; // define large errors in track proper frame (snp=0)
  //
  const double kLargeErr2Coord = 5 * 5;
  const double kLargeErr2Dir = 0.7 * 0.7;
  const double kLargeErr2PtI = 30.5 * 30.5;
  ...
  for (Int_t j = lastActiveLayer + 1; j--;) { // Layer loop
    ...
    if (!isVertex) {
      ...
      // if (layer->IsInGap(phi)) {
      //   if (DetectorK::verboseR)
      // printf("Track at phi=%.2f lies in a phi gap of layer %s, skipping.\n",
    ...
    //
    if (!isVertex && !layer->isDead && !layer->isTempDead) {
      //
      // create fake measurement with the errors assigned to the layer
      // account for the measurement there
      double meas[2] = {probTr.GetY(), probTr.GetZ()};
    ...
    // correct for materials of this layer
    // note: if apart from MS we want also e.loss correction, the density*length
    // should be provided as 2nd param
    if (layer->radL > 0 &&
        !probTr.CorrectForMeanMaterial(layer->radL, 0, mass, kTRUE)) {
      printf("Failed to apply material correction, X/X0=%.4f\n", layer->radL);
      probTr.Print();
      exit(1);
    }
  ...
  // BACKWORD TRACKING +++++++++++++++++
  // number of layers is quite low ... efficiency calculation was probably
  // nonsense Tracking outward (backword) to get reliable efficiencies from
  // "smoothed estimates"

  // For below, see paper, NIM A262 (1987) p.444, eqs.12.
  // Equivalently, one can simply combine the forward and backward estimates.
  // Assuming pf,Cf and pb,Cb as extrapolated position estimates and errors from
  // fwd and bwd passes one can use a weighted estimate Cw = (Cf^-1 + Cb^-1)^-1,
  // pw = Cw (pf Cf^-1 + pb Cb^-1). Surely, for the most extreme point, where
...
Bool_t DetectorK::ExtrapolateToR(AliExternalTrackParam *probTr, double rTgt,
  ...
  printf("Xcurr: %.2f Xdest: %.2f %d Icurr:%d Itgt:%d Rcurr:%.2f Rdest:%.2f\n",
         ...
         ((CylLayerK*)fLayers.At(lrCurr))->radius,
  ((CylLayerK*)fLayers.At(lrTgt))->radius );
  */
  //
...
TGraph *DetectorK::GetGraphPointingResolution(Int_t axis, Int_t color,
  ...
  // graph->SetMaximum(50.1) ;
  graph->SetMaximum(800.1);
  graph->GetXaxis()->SetTitle("Transverse Momentum (GeV/c)");
  graph->GetXaxis()->CenterTitle();
  graph->GetXaxis()->SetNoExponent(1);
  graph->GetXaxis()->SetMoreLogLabels(1);
...
TGraph *DetectorK::GetGraphRecoPurity(Int_t particle, Int_t color,
  ...
  //  Double_t lambda = TMath::Pi()/2.0
  //  - 2.0*TMath::ATan(TMath::Exp(-1*fAvgRapidity));
...
TGraph *DetectorK::GetGraphImpactParam(Int_t mode, Int_t axis, Int_t color,
  ...
  for (Int_t ip = 0; ip < trackRes->GetN(); ip++) {
    ...
    if (mode == 0)
      graph->SetPoint(ip, pt[ip],
                      TMath::Sqrt(vtxRes * vtxRes + trRes[ip] * trRes[ip]));
    else if (mode == 1)
...
void DetectorK::MakeAliceCurrent(Int_t AlignResiduals, Bool_t flagTPC) {
  ...
  if (AlignResiduals == 0) {
    ...
    // itsRecoParam->SetClusterMisalErrorYBOn(0.0010,0.0030,0.0500,0.0500,0.0020,0.0020);
    // // [cm]
...
/*void DetectorK::AddPetalGapsToLayer(const char *layerName, int nPetals,
                                    double gapWidth) {
  ...
  for (int i = 0; i < nPetals; ++i) {
    double phi_start =
        (TMath::Pi() / nPetals) + i * (2 * TMath::Pi() / nPetals);
    double phi_end = phi_start + gapWidth;
    layer->AddPhiGap(phi_start, phi_end);
  }
}
*/
...
void DetectorK::MakeStandardPlots(Bool_t add, Int_t color, Int_t linewidth,
  ...
  if (!add) {
    ...
    int n = eff->GetN();
    for (int k = 0; k < n; k++) {
      printf("bin %d, efficiency %f\n", k, eff->GetPointY(k));
    }
    eff->SetName(Form("grEff%d", 1));
    eff->SetTitle("Efficiencies");
    eff->Draw("AL");
    eff->SaveAs("eff.root");
    eff->SaveAs(Form("eff%d%s.root", 1, shiftLabel.Data()));
    ...
    pointRes->Draw("L");
    pointRes->SaveAs("pointResZ.root");

    c1->SaveAs("Standard_plots.pdf");
    c1->SaveAs("Standard_plots.root");
  }
}
...
Bool_t DetectorK::GetXatLabR(AliExternalTrackParam *tr, Double_t r, Double_t &x,
  ...
  // track position >0 - go along the track (increasing R) <0 - go backward
  // (decreasing R)
  //
  // special case of R=0
  if (r < kAlmost0) {
  ...
  if (TMath::Abs(crv) > kAlmost0) { // helix
    ...
    if (TMath::Abs(y0) > kAlmost0) { // when y0==0 the x,y is unique
      ...
      if (dir == 0) { // chose the one which corresponds to smallest step
        ...
        //	if (verboseR) printf("d+: x:%+e|%+e fx: %+e d
...
Bool_t DetectorK::PropagateToR(AliExternalTrackParam *trc, double r, double b,
  ...
  while (1) {
    ...
    if (!trc->Rotate(trc->Phi())) {
      printf("Failed to rotate to local frame %f |", trc->Phi());
      trc->Print();
      return kFALSE;
    }
    ...
    if (!trc->PropagateTo(xR, b)) {
      printf("Failed to propagate to X=%f for R=%f snp=%f | iter=%d\n", xR, r,
             snp, iter);
      trc->Print();
      return kFALSE;
    }
    double rcurr2 = xR * xR + trc->GetY() * trc->GetY();
    if (TMath::Abs(rcurr2 - rr) < kTiny || rr < kTiny)
      break;
    //
    //    printf("new it%d for r=%f (xR=%f) rcurr=%f snp:%f alp:%f\n",iter,
    //    r,xR,TMath::Sqrt(rcurr2),trc->GetSnp(),trc->GetAlpha());
    if (++iter > 8) {
  ...
  if (r>kTiny) {
    ...
    if (!trc->Rotate(phi)) {printf("Failed to rotate to %f to propagate to
