...
void fatInit_geometry_ML_baseline_VD_realistic(float field = 2.0, float rmin = 20.)
{
  // Layer properties
  ...
  fat.AddLayer((char*)"L03", 7., x0OT, xrho, resRPhiOT, resZOT, eff);
  fat.AddLayer((char*)"L04", 11., x0OT, xrho, resRPhiOT, resZOT, eff);
  fat.AddLayer((char*)"L05", 15., x0OT, xrho, resRPhiOT, resZOT, eff);

  fat.AddLayer((char*)"iTOF", 19., 3. * x0OT, xrho, 0.03, 0.03, 1.0);

  fat.AddLayer((char*)"L06", 20., x0OT, xrho, resRPhiOT, resZOT, eff);
...
void lutWrite_geometry_ML_baseline_VD_realistic(const char* filename = "lutCovm.dat", int pdg = 211, float field = 2.0, float rmin = 20.)
{

  // init FAT
  fatInit_geometry_ML_baseline_VD_realistic(field, rmin);
  // write
